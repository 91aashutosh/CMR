window.addEventListener('load', async () => {
    ...
    try {
        // Fetch audience list
        const audienceResponse = await axios.post(apiUrl('customers/getList'), {
            spends,
            visits,
            noVisitMonths
        });
        ...
        const campaignsResponse = await axios.get(apiUrl('campaigns'));
    ...
    document.getElementById('campaign-form').addEventListener('submit', async (event) => {
        ...
        try {
            const response = await axios.post(apiUrl('campaigns'), { message });
    ...
    document.getElementById('send-message').addEventListener('click', async () => {
        try {
            ...
            for (const customer of audienceList) {
                await axios.post(apiUrl('communication-log'), {
